...
from core.decision.output_gate import process, confirm_action, FinalOutput
from core.decision.action_resolver import ActionRequest
from core.session.session import get_session, end_session, Session
...
def _cmd_status() -> None:
    ...
    # Memory stats
    try:
        from core.memory.manager import stats as mem_stats
        m = mem_stats()
        total = sum(m.values())
    ...
    # Action stats
    try:
        from core.capabilities.action.action_logger import get_stats as action_stats
        a = action_stats()
...
def _cmd_recall(keyword: str) -> None:
    ...
    try:
        from core.memory.manager import recall
...
def _cmd_forget(args: str) -> None:
    ...
    try:
        from core.memory import manager as mm
        forget_fn = {
...
def _cmd_actions(args: str) -> None:
    ...
    if args.strip() == "stats":
        try:
            from core.capabilities.action.action_logger import get_stats
            s = get_stats()
            print()
    ...
    try:
        from core.capabilities.action.action_logger import get_recent
...
def _cmd_app_add(args: str) -> None:
    ...
    try:
        from core.capabilities.action.app_discovery import add_custom_app
...
def _cmd_export() -> None:
    ...
    try:
        from runtime.config.backup import export_all
        zip_path = export_all()
...
def _shutdown() -> None:
    ...
    try:
        from core.memory.manager import stop_decay_engine
        stop_decay_engine()
    except Exception:
        pass

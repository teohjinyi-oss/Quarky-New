...
from AppStudio.Config import ACTIONS_DIR
...
def _undo_file_restore(info: UndoInfo) -> ActionResult:
    ...
    try:
        from MAIINNN.Functions.action.file_manager import undo_delete
        return undo_delete()
...
def _undo_volume(info: UndoInfo) -> ActionResult:
    ...
    try:
        from MAIINNN.Functions.action.system_control import undo_volume
        return undo_volume()
    except Exception as e:
...
def _undo_brightness(info: UndoInfo) -> ActionResult:
    ...
    try:
        from MAIINNN.Functions.action.system_control import undo_brightness
